...
// Chroma image paths look like ".../champion-chroma-images/<championId>/<id>.png"
function championIdFromImage(imagePath) {
    const m = imagePath && imagePath.match(/champion-chroma-images\/(\d+)\//);
    return m ? Number(m[1]) : null;
}

function normalize(it) {
    const chromaObj = it.chromaData && typeof it.chromaData === "object" ? it.chromaData : null;
    return {
        ...
        // Chroma-specific enrichment (null for non-chroma items):
        championId: it.subType === "chroma" ? championIdFromImage(it.imagePath) : null,
        chromaName: chromaObj ? chromaObj.name : null,
    };
}
